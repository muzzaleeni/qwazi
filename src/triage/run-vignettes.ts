...
import { TriageInput, TriageLevel } from "./types";
...
function main() {
  ...
  for (const file of files) {
    ...
    const result = evaluateTriage(vignette.input, rules);
  ...
  process.stdout.write(
    `\nVignette summary: ${passed} passed, ${failed} failed, ${files.length} total.\n`
  );
...
function validate(vignette: Vignette, result: ReturnType<typeof evaluateTriage>): string[] {
  const errors: string[] = [];
...
function printUsage() {
  process.stdout.write(
    [
      "Usage:",
      "  npm run triage:test -- [--dir <vignettes-dir>] [--rules <path-to-rules-json>]",
      "",
      "Example:",
      "  npm run triage:test -- --dir test/vignettes",
    ].join("\n")
  );
  process.stdout.write("\n");
}
